...
const calculateLevel = (count) => {
    if (count < 5) return "Novato";
    if (count < 15) return "Aprendiz";
    if (count < 30) return "Arquitecto";
    return "Maestro";
};
...
const progressBar = (currentStep, isDark) => {
    ...
    <div class="w-full mb-8 px-1 mt-4">
        ...
        <div class="h-2 rounded-full overflow-hidden ${isDark ? 'bg-stone-800' : 'bg-stone-300'}">
            <div class="h-full transition-all duration-700 ease-in-out relative ${isDark ? 'bg-indigo-500' : 'bg-stone-600'}" style="width: ${percent}%"></div>
        </div>
    </div>`;
};
...
const calendar = (history, currentDate, selectedDateStr, isDark) => {
    ...
    <div class="p-4 sm:p-6 rounded-2xl border mb-6 ${isDark ? 'bg-stone-950 border-stone-800' : 'bg-[#FAF9F6] border-stone-200'}">
        ...
        <div class="grid grid-cols-7 gap-2 text-center text-[10px] sm:text-xs font-bold opacity-50 mb-2"><div>D</div><div>L</div><div>M</div><div>M</div><div>J</div><div>V</div><div>S</div></div>
        <div class="grid grid-cols-7 gap-1 sm:gap-2">${daysHtml}</div>
    </div>`;
};
...
const historyCard = (entry, isDark) => {
    ...
    const sections = [
        ...
        <div class="mb-3 break-words">
            <h4 class="text-xs font-bold uppercase tracking-wider opacity-60 mb-1 flex items-center gap-1"><i data-lucide="${s.icon}" class="w-3 h-3"></i> ${s.label}</h4>
    ...
    <div class="rounded-xl border shadow-sm mb-6 overflow-hidden fade-in ${c.bgCard}">
        ...
        <div class="p-5"><div class="grid md:grid-cols-2 gap-4">${sections}</div>
            ${entry.feedback ? `<div class="mt-4 p-3 rounded-lg text-sm flex gap-3 ${isDark ? 'bg-amber-900/20 text-amber-200' : 'bg-amber-50 text-stone-700'}"><i data-lucide="brain" class="w-5 h-5 flex-shrink-0 mt-1"></i><div>${entry.feedback}</div></div>` : ''}
        </div>
    </div>`;
};
...
export const Render = {
    welcome: (state) => {
        ...
        // RECUPERADO: Contador de Sesiones y Nivel
        const count = state.history.length;
        const level = calculateLevel(count);

        return `
        <div class="text-center max-w-lg mx-auto fade-in pt-4 pb-20">
            ...
            <h1 class="text-5xl font-bold mb-4 font-serif ${c.textMain}">NeuroLog</h1>
            
            <!-- BADGES DE NIVEL Y SESIONES RECUPERADOS -->
            <div class="flex justify-center gap-2 mb-8">
                <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${state.isDarkMode ? 'border-stone-700 text-stone-400' : 'border-stone-300 text-stone-600'}">
                   ${level}
                </span>
                <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border bg-amber-100 text-amber-800 border-amber-200">
                   ${count} Sesiones
                </span>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <button onclick="window.app.startSession('MORNING')" class="group border-2 p-6 rounded-xl text-left hover:shadow-md transition-all ${c.bgCard} hover:border-amber-400 active:scale-95">
                ...
                <button onclick="window.app.startSession('EVENING')" class="group border-2 p-6 rounded-xl text-left hover:shadow-md transition-all ${c.bgCard} hover:border-indigo-400 active:scale-95">
        ...
        <div class="max-w-3xl mx-auto fade-in pb-20">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex gap-2">
                    <button onclick="window.app.exportLocalData()" class="p-2 rounded-full border ${c.bgCard}"><i data-lucide="download" class="w-5 h-5"></i></button>
                    <button onclick="window.app.handleDriveSync()" class="p-2 rounded-full border ${c.bgCard} ${state.isSyncing ? 'animate-spin' : ''}"><i data-lucide="refresh-cw" class="w-5 h-5"></i></button>
                </div>
            </div>
